      ...
      <main className="mx-auto w-full max-w-[1200px] px-[5vw] pb-24 pt-[118px]">
        <header className="border-b border-white/10 pb-8">
          <div className="flex flex-wrap items-start justify-between gap-5">
            <div>
              <p className="text-[0.62rem] uppercase text-white/55">Casos</p>
              <h1 className="mt-3 text-[clamp(2.4rem,8vw,5.8rem)] font-black uppercase leading-[0.9] text-[#ece8e2] [font-family:var(--font-helvetica-neue)]">
                Criterio aplicado
              </h1>
            </div>
            <div className="flex flex-col items-start gap-2 text-left sm:items-end sm:text-right">
              <p className="text-[0.6rem] uppercase text-white/60">
                Disponible para proyectos
              </p>
              <p className="text-[0.68rem] uppercase text-white/40">
                Actualizado 2026
              </p>
            </div>
          </div>
          ...
          <div className="mt-7 flex flex-wrap items-center gap-3">
            <Link
              href="/work"
              className="inline-flex items-center rounded-full border border-white/20 bg-white/10 px-5 py-2 text-[0.68rem] font-semibold uppercase text-white transition-colors duration-200 hover:bg-[#f7b7ff] hover:text-black"
            >
              Todo
            </Link>
            <Link
              href="/branding"
              className="inline-flex items-center rounded-full border border-white/20 bg-transparent px-5 py-2 text-[0.68rem] font-semibold uppercase text-white/90 transition-colors duration-200 hover:bg-[#f7b7ff] hover:text-black"
            >
              Branding
            </Link>
            <Link
            ...
            >
              <Link href={`/work/${project.slug}`} className="block">
                <div className="relative overflow-hidden border border-white/10 bg-[#0f0f0f]">
                  ...
                  <span className="absolute right-4 top-4 inline-flex items-center rounded-full border border-white/25 bg-black/30 px-3 py-1 text-[0.58rem] uppercase text-white/85 backdrop-blur-sm">
                    Ver caso
                  </span>
                </div>
